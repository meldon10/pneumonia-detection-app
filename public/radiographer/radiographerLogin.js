...
document.querySelector('#loginButton').addEventListener('click',(event)=>{
    ...
    fetch(`/Radiographer/signin/login`,{
        method : 'POST',
    headers: {
    ...
    body : JSON.stringify(data),
        ...
        if (response.status === 200){
            ...
            response.json().then((data)=>{
                ...
                if(data.token){
                    window.location.href = `/radiographer/Account?token=${data.token}`
           ...
           document.querySelector('#userError').innerHTML=`<p style="color: red; font-size:12px; margin-bottom:2px;">Invaild username or password</p>`
        }
...
document.querySelector('#submitDetails').addEventListener('click',(event)=>{
    ...
    // PUT ERROR HANDLING FOR MISSING DATA
...
if(firstName.value && lastName.value && email.value && password.value && confPassword.value && address.value && mobile.value && country.value){
    ...
    fetch(`/Radiographer/accountDetails`,{
		method : 'POST',
		headers: {
			'Content-Type' : 'application/json'
		},
