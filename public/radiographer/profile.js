...
fetch(`/Radiographer/Account/profile`,{
    ...
    headers: {
        ...
        response.json().then((data)=>{
            ...
            radiographerName.innerHTML = `<strong style="font-family: Arial;color:#4f6ecc">RADIOGRAPHER</strong><br>${data.fname}&nbsp;${data.lname}`
        })
 })
...
function passwordValidate(password){
 ...
 document.querySelector('#saveSettings').addEventListener('click',(event)=>{
    ...
    if(fname.value && lname.value && email.value && password.value && address.value && mobile.value && country.value)
    {
      ...
      if(output !== true){
            ...
            if(output === "length error"){
                  flag=false
                  document.querySelector("#errorPassword").innerHTML =`<p style="color: red;margin-bottom:2px;margin-top:2px;margin-left:3px;font-size:14px;">Password must be 8 characters long</p>`
    ...
    fetch(`/Radiographer/Account/updateProfile`,{
    method : 'POST',
    headers: {
        'Authorization': bearer,
        'Content-Type': 'application/json',
    },
    body : JSON.stringify(radiographerData)
