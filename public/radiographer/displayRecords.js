  ...
  var patients =  document.querySelectorAll('.patientImage')
    for(var i =0 ;i<patients.length;i++){
        patients[i].addEventListener('click',(event)=>{
            ...
            fetch(`/Radiographer/Account/getPatientImage?email=${email}`,{
                method : 'GET',
                headers: {
                'Content-Type' : 'application/json'
                }
...
fetch(`/Radiographer/Account/getPatient`,{
    method : 'GET',
    headers: {
    'Content-Type' : 'application/json'
    }
    ...
    response.json().then((data)=>{
        ...
        for(var i=0;i<patientRecords.length;i++){
            ...
            html = '<tr id="patientRecord-%id%"><td class="patientImage">%Name%</td><td>%Email%</td><td>%Mob%</td><td>%Age%</td><td>%Date%</td><td><button class="btn">Delete</button></td></tr>'
...
document.querySelector(".records").addEventListener("click",(event)=>{
    ...
    if(index[0] === "patientRecord" && parseInt(index[1]) >= 0)
      {  
        ...
        patientRecords =  patientRecords.filter((ele)=>{
        ...
        fetch(`/Radiographer/Account/deletePatient?record=${temp}`,{
            method : 'GET',
            headers: {
            'Content-Type' : 'application/json'
            }
